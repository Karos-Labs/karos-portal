...
and it was; the registration was the bug). `isUnlistedAgent` in `custom-agent-launch.ts` is the one predicate every roster reads (`isSubAgent` from `parentKey`, OR `isSupersededAgentKey` for the e10 generation — two genuinely different reasons, deliberately not conflated: a superseded agent has no parent, so giving it one to hide it would be a lie in the data about what it is), and a standalone
...
- **Auto-posting / LinkedIn OAuth write scopes as a NEW mechanism.** Still
  ...
  **PARTIALLY IMPLEMENTED 2026-09-21, opt-in, per client:** the EXISTING
  ...
  `linkedin` `ClientIntegration` is one shared credential (the same one
