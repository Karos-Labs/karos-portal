...
/**
 * Flags each task whose linked custom agent (metadata.customAgentId) is
 * currently disabled — the fact both the card's warning banner and the
 * ticket's "Pause task" control need, and the reason a task can sit active
 * with nothing able to run it. `CustomAgent.enabled` is global, not
 * client-scoped, so one lookup here covers every task on the page.
 */
function withAgentAvailability<T extends ClientTask>(
  tasks: T[],
  disabledAgentIds: ReadonlySet<string>,
): (T & { _agentDisabled?: boolean })[] {
  return tasks.map((t) => {
    const agentId = resolveTaskCustomAgentId(t);
    return agentId && disabledAgentIds.has(agentId) ? { ...t, _agentDisabled: true } : t;
  });
}
...
export async function TasksBody({ user, viewClientId }: { user: AppUser; viewClientId?: string }) {
  ...
  if (scopedClientId) {
    const [rawTasks, activityLogs, jobs, report, rawAssets, umbrellas, client, customAgents] =
      await Promise.all([
    ...
    const disabledAgentIds = new Set(customAgents.filter((a) => !a.enabled).map((a) => a.id));
    const tasks = withAgentAvailability(rawTasks, disabledAgentIds);
  ...
  const [allTasks, clients, customAgents] = await Promise.all([
  ...
  const annotatedTasks: (ClientTask & { _clientName?: string; _agentDisabled?: boolean })[] =
    withAgentAvailability(
      allTasks.filter((t) => clientMap.has(t.clientId)),
      disabledAgentIds,
    ).map((t) => ({
      ...t,
