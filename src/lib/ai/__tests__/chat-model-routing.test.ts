...
import { aiFor, ProviderWiringError, usageFor, vendorForRole } from "../provider";
...
describe("CHAT_MODEL_OPTIONS is the allowlist and matches the ticket's target", () => {
  ...
  it("gives the picker three distinct behaviours: Auto routes, Fast is Gemini, Quality is Haiku", () => {
    ...
    // The `@` spelling, which is what makes this a real assertion rather than a
    // restatement: Vertex separates a dated snapshot with `@`, so the
    // first-party `claude-haiku-4-5-20251001` is not a model id there and 404s.
  ...
  it("puts the whole chat on ONE vendor account — no option may bill to the first-party Anthropic key", () => {
    // The reason this file changed on 2026-09-23. Fast ran on Vertex (Gemini)
    // and Quality on `ANTHROPIC_API_KEY`, so one surface arrived on two
    // invoices against two credentials. The price to the client is unmoved:
    // `CHAT_MESSAGE_CREDITS` is keyed on the model FAMILY, not on which
    // infrastructure carried the call.
    for (const key of CHAT_MODEL_KEYS) {
      expect(CHAT_MODEL_OPTIONS[key].vendor, key).not.toBe("anthropic");
    }
  });

  it("keeps the follow-up chips on the same vendor as the answer they belong to", () => {
    // `chat.followups` is picked by TIER, never through the picker, so without
    // a pin it follows `defaultVendor()` back to Anthropic — the two-invoice
    // split reappearing one function below the one that closed it. Invisible
    // either way: the chips look right.
    expect(roleSpec("chat.followups").pinnedTo?.vendor).toBe("vertex");
    expect(vendorForRole("chat.followups", "anthropic")).toBe("vertex");
  });
