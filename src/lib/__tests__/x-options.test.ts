...
describe("optionsLead", () => {
  ...
  it("does not ask a client to pick one of nothing", () => {
    // The zero branch, and the empty array that reaches it IS live: the
    // already-picked path in client-agent-rows.ts builds `today = { slotId,
    // options: [], pickedDirection }`, and `pickedDirection` can be null (it falls
    // back to `refLaneLabel`, which returns `string | null`), in which case the
    // panel mounts the picker rather than OptionPicked — with nothing in it. One
    // `if (options.length === 0) return null` is all that stands between that and
    // this sentence, so this pins the SENTENCE, which is what was wrong: the
    // plural branch composed "0 directions to choose from. Pick one…".
    const lead = optionsLead(0);
    expect(lead).not.toContain("0 directions");
    expect(lead).not.toContain("choose from");
    expect(lead).not.toContain("Pick one");
    // Non-vacuous: the neighbouring count DOES ask for a pick and DOES name its
    // number, so the three negatives above are about the zero branch and not
    // about a function that stopped saying either.
    expect(optionsLead(2)).toContain("2 directions");
    expect(optionsLead(2)).toContain("Pick one");
  });

  it("still says picking is free, whichever day it is", () => {
    // The component's own rule: nothing is generated by choosing, so no copy on
    // this surface may imply a credit path. Zero included — an empty day is the
    // one where a client is most likely to wonder what a pick would cost.
